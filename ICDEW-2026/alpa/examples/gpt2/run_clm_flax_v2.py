...
PREEMPT_FILE = "/tmp/preempt"
preemption_signal = False
...
class TrainingArguments:
    ...
    cached_profile_result: str = field(default=None, metadata={"help": "A previously created profile result to avoid reprofiling."})
...
def main():
    ...
    # Data Parallel
    #method = alpa.DataParallel(num_micro_batches=training_args.num_micro_batches)
    
    # Shard Parallel
    #method = alpa.ShardParallel(num_micro_batches=training_args.num_micro_batches) 
    
    # Pipeline and shard parallelism combined, triggers dynamic programming
    layer_num=2
    method = alpa.PipeshardParallel(num_micro_batches=training_args.num_micro_batches, 
                            layer_option=alpa.AutoLayerOption(layer_num=layer_num), 
                            stage_option="auto",
                            cached_profile_result=training_args.cached_profile_result)
    
    p_train_step = alpa.parallelize(train_step,
                                    method=method,
                                    donate_argnums=(0,))
    p_eval_step = alpa.parallelize(eval_step, method=alpa.FollowParallel(
                                    p_train_step,
    ...
    for epoch in epochs:
        ...
        for step in tqdm(range(steps_per_epoch), desc="Training...", position=1, leave=False):
            ...
            if (cur_step % training_args.save_steps == 0 and cur_step > 0) or (os.path.isfile(PREEMPT_FILE)):
                ...
                if os.path.isfile(PREEMPT_FILE): # Preemption signal
                    preemption_signal = True
                    break
        
        if preemption_signal: # Preemption signal
            os.remove(PREEMPT_FILE)
            epochs.write("Pretraining preempted. To continue pretraining, run this script with --model_name_or_path argument.")
            break
