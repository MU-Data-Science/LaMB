...
def main():
    ...
    if (command=="start"):
        ...
        #plt.show()
    elif (command=='simpleplot'):
        # Make sure the first line in the report is uncommented
        df = pd.read_csv("vm{}{}".format(1, "-gpu-report.out"), delimiter=r"\s+", comment='#')
        df2=df.loc[df['gpu']==0]
        df3=df.loc[df['gpu']==1]
